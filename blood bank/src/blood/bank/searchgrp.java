...
import java.sql.*;
import java.util.*;
public class searchgrp extends BloodBank
{
    ...
    s = c.createStatement();
    String ps;
            System.out.println("enter the blood group to be searched");
            ps=in.next();
            System.out.println();
ResultSet rs = s.executeQuery( "SELECT * FROM bloodtab where bg='"+ps+"';" );
while ( rs.next() )
{
int id = rs.getInt("id");
